...
const Schema = z.object({
  // Either targetUserId (single user) or broadcast=true (cron fan-out)
...
/** Build kind-specific (title, body, link) honouring user prefs. */
async function resolveContent(
  ...
  // Defaults if caller didn't pass content
  ...
  if (!override.title || !override.body) {
    if (kind === "morning_summary") {
      ...
      const total = (rows ?? []).reduce((s, r: { amount?: number | null }) => s + Number(r.amount ?? 0), 0);
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        // Auth: either logged-in user JWT (targets self) OR x-cron-secret (broadcast/internal)
        ...
        try {
          if (parsed.broadcast) {
            ...
            // Fan out to every user that has at least one device token
            const { data: users } = await supabaseAdmin
              .from("device_tokens").select("user_id");
            const uniq = Array.from(new Set((users ?? []).map((r: { user_id: string }) => r.user_id)));
          ...
          // Logged-in users can only push to themselves (unless cron)
