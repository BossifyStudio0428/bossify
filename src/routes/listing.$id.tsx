...
function ListingEditor() {
  ...
  const save = async () => {
    ...
    const { error } = isNew
      ? await supabase.from("listings" as never).insert({ ...payload, user_id: user.id } as never)
      : await supabase.from("listings" as never).update(payload as never).eq("id", id);
    setSaving(false);
  ...
  const remove = async () => {
    ...
    const { error } = await supabase.from("listings" as never).delete().eq("id", id);
    if (error) { toast.error(error.message); return; }
