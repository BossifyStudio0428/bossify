...
function buildSections(p: {
  ...
  sections.push({
    title: t("svc_section_price"),
    content: (
      <>
        {p.isFreelance && (
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold tracking-wider uppercase text-muted-foreground px-1">{t("svc_rate_type")}</label>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => p.setRateType("fixed")} className={`py-2.5 rounded-2xl text-sm font-semibold border ${p.rateType === "fixed" ? "bg-primary text-primary-foreground border-primary" : "bg-muted/40 border-border/60 text-muted-foreground"}`}>{t("svc_rate_fixed")}</button>
              <button type="button" onClick={() => p.setRateType("hourly")} className={`py-2.5 rounded-2xl text-sm font-semibold border ${p.rateType === "hourly" ? "bg-primary text-primary-foreground border-primary" : "bg-muted/40 border-border/60 text-muted-foreground"}`}>{t("svc_rate_hourly")}</button>
            </div>
          </div>
        )}
        <Field label={`${t("price")}${p.isFreelance && p.rateType === "hourly" ? " " + t("svc_per_hour") : ""}`} value={p.price} onChange={p.setPrice} type="number" />
        {p.showDur && (
  ...
  if (p.showStock || p.showVariants) {
    sections.push({
      title: t("svc_section_stock_variants"),
      content: (
        <>
          {p.showStock && (
  ...
  if (p.showAddons) {
    sections.push({
      title: t("svc_section_addons"),
      subtitle: p.isFnb ? t("svc_addons_subtitle_fnb") : t("svc_addons_subtitle_default"),
      content: (
        <div className="space-y-2">
          {p.addons.length === 0 && (
            <p className="text-xs text-muted-foreground italic px-1">{t("svc_no_addons")}</p>
          )}
          {p.addons.map((a) => (
            <div key={a.id} className="flex gap-2 items-center">
              <input value={a.name} onChange={(e) => p.updateAddon(a.id, { name: e.target.value })} placeholder={t("svc_addon_name")} className="flex-1 rounded-xl bg-muted/40 border border-border/60 px-3 py-2 text-sm outline-none focus:border-primary" />
              <input type="number" value={a.price || ""} onChange={(e) => p.updateAddon(a.id, { price: Number(e.target.value) || 0 })} placeholder={t("svc_addon_price_ph")} className="w-24 rounded-xl bg-muted/40 border border-border/60 px-3 py-2 text-sm outline-none focus:border-primary" />
          ...
          <button type="button" onClick={p.addAddon} className="w-full py-2 rounded-xl border border-dashed border-border/80 text-xs font-semibold text-primary hover:bg-primary/5 transition">
            <Plus className="inline h-3.5 w-3.5 mr-1" />{t("svc_add_addon")}
          </button>
        </div>
      ),
    });
  ...
  if (p.isEducation) {
    sections.push({
      title: t("svc_section_course"),
      content: (
        <>
          <Field label={t("svc_level")} value={p.level} onChange={p.setLevel} />
          <Field label={t("svc_intake")} value={p.intake} onChange={p.setIntake} />
          <Field label={t("svc_requirements")} value={p.requirements} onChange={p.setRequirements} multiline />
        </>
      ),
    });
  }
  ...
  if (p.isFreelance) {
    sections.push({
      title: t("svc_section_delivery"),
      content: (
        <>
          <Field label={t("svc_turnaround")} value={p.turnaround} onChange={p.setTurnaround} type="number" />
          <div className="space-y-2">
            <label className="text-[11px] font-semibold tracking-wider uppercase text-muted-foreground px-1">{t("svc_portfolio_links")}</label>
            {p.portfolioLinks.length === 0 && (
              <p className="text-xs text-muted-foreground italic px-1">{t("svc_no_links_yet")}</p>
            )}
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => { if (p.newLink.trim()) { p.setPortfolioLinks((prev) => [...prev, p.newLink.trim()]); p.setNewLink(""); } }} className="px-3 rounded-xl border border-dashed border-border/80 text-xs font-semibold text-primary hover:bg-primary/5">{t("svc_add_link")}</button>
            </div>
          </div>
        </>
      ),
