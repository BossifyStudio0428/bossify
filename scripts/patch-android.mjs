...
/**
 * Android treats `ic_launcher.png` and `ic_launcher.webp` in the same
 * mipmap-* folder as the SAME resource and fails the build with
 * "Duplicate resources". This happens when icons get regenerated by
 * different tools (Android Studio Image Asset wizard + @capacitor/assets,
 * for example). Keep .webp (smaller, what Capacitor/AS generate by
 * default) and remove the duplicate .png variants for the known launcher
 * icon names so the APK builds and uses the Bossify icon.
 */
function dedupeLauncherIcons() {
  if (!existsSync(resRoot)) {
    console.warn(`Skipped icon dedupe: ${resRoot} not found.`);
    return;
  }
  ...
  const exts = [".png", ".webp"];
  let removed = 0;
  for (const entry of readdirSync(resRoot)) {
    if (!entry.startsWith("mipmap-") && !entry.startsWith("drawable-") && entry !== "mipmap" && entry !== "drawable") {
      continue;
    }
    const dir = join(resRoot, entry);
    ...
    for (const f of files) {
      ...
      if (!launcherNames.has(base)) continue;
      if (!exts.includes(ext)) continue;
      if (!byName.has(base)) byName.set(base, []);
      byName.get(base).push(f);
    }
    for (const [base, variants] of byName) {
      if (variants.length < 2) continue;
      // Prefer keeping .webp; delete .png duplicate(s).
      const keep = variants.find((v) => v.toLowerCase().endsWith(".webp")) || variants[0];
      for (const v of variants) {
        if (v === keep) continue;
        try {
          unlinkSync(join(dir, v));
          removed++;
          console.log(`Removed duplicate launcher resource: ${join(dir, v)} (kept ${keep})`);
        } catch (e) {
          console.warn(`Could not remove ${join(dir, v)}: ${e.message}`);
        }
      }
      void base;
    }
  ...
  if (removed === 0) {
    console.log("No duplicate launcher icon resources found.");
  }
}
